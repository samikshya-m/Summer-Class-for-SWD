...
app.put('/api/events/:id', (req, res) => {
  try {
    ...
    db.prepare('UPDATE events SET name=?, description=?, date=?, time=?, location=? WHERE id=?')
...
app.post('/api/events/:id/register', (req, res) => {
  ...
  try {
    ...
    const result = db
      .prepare('INSERT INTO attendees (event_id, name, email, phone, ticket_type) VALUES (?, ?, ?, ?, ?)')
      .run(req.params.id, name, email, phone, ticketType);

    const attendee = db.prepare('SELECT * FROM attendees WHERE id = ?').get(result.lastInsertRowid);
    return res.json({ success: true, message: 'Registration confirmed!', data: attendee });
  } catch (err) {
...
app.get('/api/events/:id/stats', (req, res) => {
  try {
    ...
    const checkedIn = db
      .prepare("SELECT COUNT(*) AS count FROM attendees WHERE event_id = ? AND status = 'checked_in'")
      .get(req.params.id).count;
    return res.json({ success: true, data: { total, checkedIn, pending: total - checkedIn } });
  } catch (err) {
